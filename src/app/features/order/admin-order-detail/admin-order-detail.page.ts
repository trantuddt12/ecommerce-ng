...
@Component({
            ...
            <mat-card-content>
              <div class="order-panel-header">
                <div>
                  ...
                  <p>Snapshot thong tin nguoi mua, giao nhan, item va tong tien.</p>
                </div>
                <a mat-stroked-button [routerLink]="APP_ROUTES.adminOrders">Ve danh sach</a>
              </div>
              ...
              <div class="order-summary-grid">
                <div><strong>Khach:</strong> {{ order()!.customerName || '-' }} ({{ order()!.customerPhone || '-' }})</div>
                <div><strong>Nguoi nhan:</strong> {{ order()!.recipientName || '-' }} ({{ order()!.recipientPhone || '-' }})</div>
                <div><strong>Dia chi:</strong> {{ fullAddress() }}</div>
                <div><strong>Tracking:</strong> {{ order()!.trackingNumber || '-' }}</div>
                <div><strong>Tam tinh:</strong> {{ order()!.subtotalAmount | number: '1.0-0' }}</div>
                <div><strong>Tong cong:</strong> {{ order()!.grandTotal | number: '1.0-0' }} {{ order()!.currencyCode || 'VND' }}</div>
              </div>
          ...
          <mat-card class="order-panel order-span-4">
            <mat-card-content>
              ...
              <div class="order-summary">
                ...
                <p><strong>Cancelled at:</strong> {{ order()!.cancelledAt ? (order()!.cancelledAt | date: 'dd/MM/yyyy HH:mm') : '-' }}</p>
                <p><strong>Delivered at:</strong> {{ order()!.deliveredAt ? (order()!.deliveredAt | date: 'dd/MM/yyyy HH:mm') : '-' }}</p>
              </div>
            </mat-card-content>
          </mat-card>
        </section>
  ...
  styles: [`
    ...
    .order-hero {
      color: #fff;
      background:
        radial-gradient(circle at 78% 20%, rgba(251, 191, 36, 0.2), transparent 36%),
        linear-gradient(135deg, rgba(15, 23, 42, 0.96) 0%, rgba(67, 56, 202, 0.9) 55%, rgba(180, 83, 9, 0.86) 100%);
      border-radius: 1.2rem;
    }

    .order-hero .mat-mdc-card-content { display: grid; gap: 0.7rem; padding: 1.4rem; }
    .order-eyebrow { margin: 0; text-transform: uppercase; letter-spacing: 0.14em; font-size: 0.74rem; color: rgba(253, 224, 71, 0.95); }
    .order-hero h2 { margin: 0; font-size: clamp(1.5rem, 2.7vw, 2.1rem); }
    .order-hero p { margin: 0; color: rgba(254, 243, 199, 0.9); }
    ...
    .order-chip { background: rgba(254, 240, 138, 0.7) !important; color: #854d0e !important; }
    .order-chip-soft { background: rgba(219, 234, 254, 0.82) !important; color: #1d4ed8 !important; }
...
export class AdminOrderDetailPage {
  ...
  protected readonly inventoryBadge = computed(() => {
    const value = this.order();
    if (!value) {
      return 'Inventory not reserved';
    }

    if (value.inventoryDeductedAt) {
      return 'Inventory deducted';
    }

    if (value.inventoryReservedAt) {
  ...
  protected applyStatusUpdate(): void {
    ...
    this.orderApi
      .updateAdminStatus(current.id, payload)
      .pipe(finalize(() => this.loading.set(false)))
      .subscribe({
        next: (updated) => {
