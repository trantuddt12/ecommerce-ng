...
8. Neu thay doi frontend lien quan API, cap nhat `AI/references/api-inventory.md` trong cung task truoc khi ket thuc.
9. Neu chi sua tai lieu (`AI/`, `doc/`, `README.md`, `*.md`) va khong doi code/cau hinh runtime, khong can chay build/test code; chi can verify bang doc review hoac diff.
10. Ghi vao workspace root `AI_PROGRESS/YYYY-MM-DD.md` cac file da sua, tom tat thay doi, ket qua verify, va todo tiep theo.
11. Giu workspace root `AI_PROGRESS.md` lam file muc luc/huong dan; chi sua file nay khi doi cau truc logging.
12. Cap nhat tai lieu `AI/` neu thay doi lam doi quy tac hoac kien truc.
